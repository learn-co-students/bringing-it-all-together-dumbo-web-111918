require 'pry'
class Dog

  attr_accessor :id, :name, :breed

  def attributes (name, breed, id)
 	  dog = Dog.new(name, breed, id)
  end

  def initialize (name:, breed:, id: nil)
	  #you can put a hash in the argument with keyword:
	  @name = name
	  @breed = breed
	  @id = id
  end


  def self.drop_table
    sql = "DROP TABLE IF EXISTS dogs"

    DB[:conn].execute(sql)
   end


  def self.create_table
    sql = <<-SQL
    CREATE TABLE IF NOT EXISTS dogs (
      id INTEGER PRIMARY KEY,
      name TEXT,
      breed TEXT
    )
    SQL

    DB[:conn].execute(sql)
  end

  #We need a way to cast that data from SQL which comes in as an array,
  # into the appropriate #attributes of a dog.
  #You can even think of it as new_from_array.
  #Methods like this, that return instances of the class,
  #are known as constructors, just like .new, except that
  #they extend
  #the functionality of .new without overwriting initialize

  def save
    if self.id
  		self.update
  	else
  	  sql = <<-SQL
  	    INSERT INTO dogs(name, breed)
  	    VALUES (?, ?)
  	  SQL
  	  DB[:conn].execute(sql, self.name, self.breed)
      #binding.pry
  	  @id = DB[:conn].execute("SELECT last_insert_rowid() FROM dogs")[0][0]
      self
    end
  end


#instead of making each instance and saving later, we use create
   def self.create(name: name, breed: breed)
  	 dog = Dog.new(name: name, breed: breed)
  	 dog.save
  	 dog #end with our instance
   end


def self.new_from_db(row)
    id = row[0]
    name = row[1]
    breed = row[2]
    self.new(name: name, breed: breed, id: id)
    #return instance you made
  end


   def self.find_by_id(id)
	   sql = <<-SQL
	   SELECT *
	   FROM dogs
	   WHERE id = ?
	   LIMIT 1
	   SQL

     DB[:conn].execute(sql, id).map do |row|
  	   self.new_from_db(row)
     end.first
   end


   def self.find_or_create_by(name:, breed:)
   	 dog = DB[:conn].execute("SELECT * FROM dogs WHERE name = ? and breed = ?", name, breed)
     if !dog.empty?
    	 dog_data =dog[0]
    	 dog = Dog.new(name: dog_data[1], breed: dog_data[2], id: dog_data[0])
     else
       dog = self.create(name: name, breed: breed)
     end
     dog
   end



   def self.find_by_name(name)
	   sql = <<-SQL
	   SELECT *
	   FROM dogs
	   WHERE name = ?
	   LIMIT 1
	   SQL

     DB[:conn].execute(sql, name).map do |row|
  	   self.new_from_db(row)
     end.first
   end

  def update
  	sql = "UPDATE dogs SET name = ?, breed = ? WHERE id = ?"
    DB[:conn].execute(sql, self.name, self.breed, self.id)
  end

end
